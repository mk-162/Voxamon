...
const ProgressBar = ({ current, max, isPro }: { current: number; max: number; isPro: boolean }) => {
  const percentage = Math.min((current / max) * 100, 100);
  return (
    <div className="w-full max-w-xl mx-auto mt-4 px-4 animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-2 text-[10px] font-bold uppercase tracking-widest">
        <span className="text-cyan-400 font-mono">{Math.floor(current / 60)}:{String(current % 60).padStart(2, '0')}</span>
        <span className={isPro ? "text-purple-400" : "text-slate-500"}>
          {isPro ? "30:00 PRO limit" : "2:00 max (upgrade)"}
        </span>
      </div>
      <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden border border-slate-700/50 p-[1px] relative">
        <div
          className={`h-full rounded-full transition-all duration-1000 ease-linear ${isPro ? 'bg-gradient-to-r from-purple-500 via-blue-500 to-purple-500 bg-[length:200%_100%] animate-gradient shadow-[0_0_10px_rgba(168,85,247,0.4)]' : 'bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.4)]'
            }`}
  ...
  useEffect(() => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionRef.current.onend = () => {
        // If we didn't manually request a stop, and we haven't hit the limit, restart!
        // This provides the "30 minute rambling" support.
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center py-12 px-4 md:px-8 selection:bg-cyan-500/30 selection:text-cyan-100">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center mb-12 w-full">
        ...
        <div className="h-16 mb-4 flex flex-col items-center justify-end">
          ...
          {isRecording && (
            <ProgressBar current={recordingSeconds} max={currentLimit} isPro={isPro} />
          )}
        </div>
